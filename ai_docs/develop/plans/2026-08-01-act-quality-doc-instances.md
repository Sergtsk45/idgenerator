...
**Создан:** 2026-08-01  
**Обновлён:** 2026-08-01  
**Статус:** 🟢 Реализовано в коде (ожидает деплой/миграцию 0028 и smoke на проде)  
**Приоритет:** Высокий  
...
- **Статус**: Не начата
- **Шаги**:
  - [ ] Не скрывать материал целиком после первого добавления (пока есть невыбранные docs)
  - [ ] После выбора материала — **сразу multi-select docs** (Q7), без автоподстановки primary
  - [ ] «Добавить» → N строк D′; primary можно бейджем, но не выбирать автоматически
  - [ ] Поле партии — optional, не фильтрует список docs
  - [ ] В строке: тип+номер документа; бейдж «нет файла» если `fileUrl` пуст
  - [ ] Запрет дубля `qualityDocumentId` (Q3): UI + 409
...
| **Q7** | При добавлении материала — **сразу multi-select** документов (не авто-primary) |

### Не блокер (уточнить при реализации)
...
- Префикс «при выполнении работ применены» скорее всего уже в шаблоне АОСР — генератор отдаёт хвост `материал + docs`; сверить с `server/templates/aosr/` при INST-007.
