...
### Добавлено
- В диалоге материалов задачи действие «Создать новый материал» (доступно при пустом и непустом списке).
- `MaterialWizard` поддерживает необязательные `onCreated`, `initialSource`, `skipSourceStep` и возвращает `CreatedMaterialResult`.
- После создания материал автоматически добавляется к текущей задаче через `replaceTaskMaterials` (с учётом split-синхронизации).

### Изменено
- Auth/ownership для маршрутов нового сценария: `taskMaterials.*`, `projectMaterials.list/create/get`, `materialBatches.create`.

### Исправлено
- Пользователю больше не нужно уходить в `/source/materials`, чтобы добавить отсутствующий материал в задачу.
- Повторное «Готово» после ошибки привязки к задаче повторяет только `replaceTaskMaterials` и не создаёт второй материал, партию или документ.
- «Готово» заблокировано с индикатором загрузки на всём submit-пайплайне, включая создание партии и привязку к задаче.
- Повторная привязка одного `projectMaterialId` не дублирует строку в локальном списке задачи.

---

## [2026-08-01] - Несколько документов качества на материал в акте (D′)

### Добавлено
- Unique `task_materials (task_id, project_material_id, quality_document_id)`; партия вне unique.
- `acts.attachments_manual` и `POST /api/acts/:id/document-attachments/reset-from-usages`.
- Multi-select документов при добавлении материала в задачу графика (N строк D′).
- Редактор приложений в карточке акта: добавить/убрать, сброс к материалам; клик по строке без PDF → upload.
- Группировка текста п.3 АОСР: перечень docs одного материала в одной фразе.

### Изменено
- `generate-acts` не перезаписывает приложения, если `attachments_manual = true`.
- Партия и документы качества трактуются как независимые оси.

---

## [2026-08-01] - Sticky-шапка дат Gantt при вертикальной прокрутке

### Изменено
- Шкала месяцев/дней в `/schedule` закреплена (`sticky top-0`) внутри скроллера таймлайна и остаётся видимой при прокрутке вниз.
- Уменьшены вертикальные отступы строк месяцев и дней для более тонкой шапки.

---

## [2026-08-01] - Компактная левая колонка графика Ганта

### Изменено
- В строке задачи дата заменена на всегда видимый код позиции; объём с единицей перенесён к метаданным, а отсутствующий объём обозначается тире.
- Высота основной строки уменьшена с 88 до 72 px без изменения auxiliary-строк и расчёта выравнивания полос Ганта.
- Ширина колонки кода сужена до минимума под трёхзначное число (`w-[3ch]`); более длинный код обрезается с полным значением в `title`.

---

## [2026-08-01] - Штамп «Копия верна» в PDF-приложениях

### Добавлено
- Роль `copy_certifier` в исходных данных объекта с полями ФИО и должности; старые payload без роли совместимы и получают пустое значение.
- Векторный штамп «Копия верна» 70 × 35 мм: кириллица Roboto, фиксированный угол −12°, рамка, линия подписи, прозрачность, best-effort зернистость и обработка длинного текста/малых страниц.
- Runnable-check роли, геометрии штампа, кириллицы, многостраничного пакета и неизменности исходных PDF.

### Изменено
- При экспорте приложений штамп накладывается на каждую страницу всех PDF-вложений из `act_document_attachments`; титульная страница остаётся без штампа.
- Позиционирование учитывает видимую CropBox и поворот страницы; пустые ФИО или должность заменяются подчёркиваниями.
- В UI ответственных лиц появился заверитель копий; для него показываются только ФИО и должность.

---

## [2026-07-31] - Раздельный экспорт приложений к акту: сервер

### Добавлено
- `server/actAttachmentsPdf.ts` собирает отдельный PDF-пакет: титульная страница через существующий pdfmake-пайплайн, затем PDF-документы из `act_document_attachments` в порядке `orderIndex`.
- `POST /api/acts/:id/export-attachments` с авторизацией и проверкой текущего объекта возвращает ссылку на пакет и пользовательское имя `Акт_{номер}_приложения.pdf`.
- В карточке акта экспорт разделён на «Экспорт акта» с прежним выбором шаблона и независимый «Экспорт приложений»; действие недоступно при пустом списке документов.
- Контракт ошибок экспорта: `409` для пустого комплекта и `422` со списком отсутствующих, повреждённых, неподдерживаемых или превысивших лимит документов.
- Runnable-check порядка страниц, дедупликации, проблемных файлов, внешних URL, лимитов и API-контракта.

### Изменено
- В `server/pdfGenerator.ts` общий вызов pdfmake выделен в `generatePdfBuffer` и переиспользуется титульной страницей пакета.
- Подключён `pdf-lib`; экспорт ограничен 100 документами и суммарным размером 200 МБ, внешние URL сервером не запрашиваются.
- UI показывает отдельный индикатор сборки приложений и перечень проблемных документов из ответа `422`.

---

## [2026-07-31] - Архив завершённых tasktracker-файлов

### Изменено
- Завершённые трекеры перенесены в `docs/archive/`: `tasktracker2.md`, `tasktracker3.md`, `tasktracker3-split-task.md`, `tasktreckerroutes.md`.
- В `docs/archive/` также: отчёты (`multi-auth-completion-report`, `split-task-*-report`), one-shot setup (`post-reboot-summary`, `setup-after-reboot`), techspec’и по уже сделанным фичам (`techspec_schedule`, `techspec_quality_statuses_schedule`).
...
### Исправлено
- Левая колонка и полосы Ганта теперь используют один набор `filteredTasks`; фильтры «С актом»/«Без акта» больше не создают вертикальный сдвиг.
- Раскрытые auxiliary-строки учитываются в общей высоте и координатах таймлайна; накопительный сдвиг от границы строки устранён.
- Удалена горизонтальная сетка с фиксированным шагом `88px`, которая не соответствовала строкам переменной высоты.
- Вспомогательные позиции `N.x` группируются с ближайшей предшествующей `N` только внутри той же коллекции ВОР; коллизии локальных `orderIndex` между коллекциями больше не перемешивают родителей и подстроки.
- Bootstrap из ВОР ограничен коллекциями текущего объекта и не принимает `workIds` другого объекта или график с несовместимым типом источника.

### Изменено
- Общий порядок ВОР детерминирован ключом `(workCollectionId, orderIndex, code natural, id)`; глобальная перенумерация `works.orderIndex` не требуется.
- Полный bootstrap удаляет legacy auxiliary/out-of-scope задачи и один раз нормализует порядок оставшихся main-задач; частичный bootstrap не удаляет посторонние задачи.
- Ответ `POST /api/schedules/:id/bootstrap-from-works` содержит `removed`; UI сообщает количество удалённых legacy-задач.

### Добавлено
- `client/src/lib/schedule-row-layout.ts` — чистый расчёт вертикальных координат видимых строк Ганта.
- Regression-тесты layout, collection-local группировки и object-scoped bootstrap.

### Проверено
- `npm test`, `npm run check`, `npm run build`, `git diff --check`.
- Production repair schedule `7` не выполнялся: перед удалением legacy-задач требуется preflight пользовательских актов, split-задач, материалов и документов.
...
### Добавлено
- `migrations/0026_documents_object_id.sql` — добавлена колонка `documents.object_id`, FK на `objects`, индексы, backfill по `document_bindings`/материалам/партиям и CHECK согласованности `scope` ↔ `object_id`.
- `migrations/0027_documents_audit_user_ids.sql` — добавлены nullable audit-ссылки `created_by_user_id` и `updated_by_user_id` для документов.
- `GET /api/documents?viewMode=project|global|all`, `PATCH /api/documents/:id`, `PATCH /api/documents/:id/scope`.
- `tests/documents-scope-contract.test.ts` и `tests/documents-object-scoping-implementation.test.ts` — покрытие контрактов, object-scoped видимости и SQL-миграции.

### Изменено
- Реестр `/source/documents` разделён на сегменты `Проект / Глобальные / Все`; в `Все` документы помечаются бейджем `Проект`/`Глобальный`.
- Создание, редактирование, удаление и действие «Сделать глобальным» доступны прямо из реестра документов.
- Счётчик «Документы качества» показывает проектные и глобальные отдельно; выбор существующих документов в материалах использует проектные документы текущего объекта + глобальные с фильтрами и scope-бейджами.
- Backend больше не отдаёт весь реестр документов: проектные документы фильтруются по текущему объекту, глобальные видны всем объектам владельца.
- `PATCH /api/documents/:id` сохраняет `docType`; list API/hooks используют только `viewMode`, без legacy alias `scope`.
- `PATCH /api/documents/:id/scope` поддерживает только публикацию project → global; global → project запрещён, чтобы общий документ не исчезал из других объектов.
- `POST/PATCH /api/document-bindings` переведены на `appAuth + resolveCurrentObject` и проверяют принадлежность документа и target текущему объекту.

### Исправлено
- Новый объект больше не видит проектные документы другого объекта.
- Документ любого scope нельзя удалить, если он используется в актах или task materials; API возвращает понятный `409`.
- Чистая цепочка миграций на `0019` получила явного заблокированного legacy-владельца для seed/orphan объектов перед `objects.user_id SET NOT NULL`.
...
### Исправлено
- `client/src/pages/Schedule.tsx`: при подтверждении конфликта `actNumberAssign`/`actNumberChange` задача теперь сразу сохраняется с `updateAllTasks: true` — ранее обновлялось только поле формы без фактического сохранения, что приводило к повторному срабатыванию 409 при следующем нажатии «Сохранить»

## [2026-03-09] - Удаление сметы теперь удаляет и график работ

### Изменено
- `server/storage.ts`: при удалении сметы с флагом `resetScheduleIfInUse` связанный график теперь **удаляется** (вместо сброса на `sourceType="works"`)
- `server/routes.ts`: обновлено сообщение 409 — описывает удаление графика, а не его сброс
- `client/src/pages/Works.tsx`: обновлены текст диалога подтверждения, кнопка («Удалить смету и график») и тост-сообщение

## [2026-03-09] - Исправление 401 на странице ВОР/Сметы

### Исправлено
- `use-estimates.ts`: все `fetch`-вызовы (list, get, import, delete) теперь передают заголовок `Authorization: Bearer <token>` через `createApiHeaders()` — ранее запросы уходили без токена и получали 401
- `use-work-collections.ts`: аналогичное исправление для всех операций с коллекциями ВОР
- `use-works.ts`: аналогичное исправление для списка, создания, импорта и удаления работ
...
### Добавлено
- Голосовой ввод на главной странице чата: кнопка микрофона теперь записывает голос и транскрибирует через OpenAI Whisper API
- API `POST /api/voice/transcribe` — загрузка аудио (FormData), транскрипция через Whisper, возврат текста
- Хук `useVoiceRecorder` — запись через MediaRecorder API, авто-стоп через 60 сек, поддержка audio/webm, audio/ogg, audio/mp4
- Визуальная обратная связь: пульсирующая красная кнопка, таймер записи, спиннер при распознавании
- i18n переводы (ru/en) для всех сообщений голосового ввода

### Безопасность
- Rate limiting (10 запросов/мин) на voice endpoint
- Auth middleware (JWT/Telegram initData) обязателен
- Multer fileFilter проверяет MIME-тип (audio/*), лимит 10 MB
- Буфер аудиофайла обнуляется после обработки (защита от утечки памяти)
- Generic error messages (детали OpenAI не утекают клиенту)
- AbortController для отмены fetch при размонтировании компонента
