...
- **Экспорт приложений**: отдельная кнопка в карточке акта вызывает `POST /api/acts/:id/export-attachments`, который собирает титульную страницу и PDF-файлы только из `act_document_attachments`. Каждая страница всех вложений получает визуальный штамп «Копия верна» с данными `copy_certifier` текущего объекта; титульная страница не штампуется. Порядок определяется `orderIndex`, дубликаты `documentId` удаляются; внешний URL, отсутствующий или повреждённый файл блокирует выдачу неполного комплекта.
- **Несколько документов качества на материал (D′)**: партия и документы качества ортогональны. В задачу/акт можно добавить несколько строк одного материала с разными `qualityDocumentId` (unique `(task, material, document)`). При добавлении материала в графике сразу multi-select docs. В п.3 АОСР docs одного материала группируются в одну фразу (`термометр … паспорт …0091, паспорт …0096`). Приложения правятся и в графике (через generate-acts), и в карточке акта; флаг `acts.attachments_manual` защищает ручной список от перезаписи. Клик по строке без PDF в карточке акта открывает загрузку файла. ADR: `ai_docs/develop/architecture/ADR-2026-08-01-act-quality-docs-multirow.md`.
...
- `server/` — бэкенд (Express)
  ...
  - `server/actAttachmentsPdf.ts` — титульная страница, штамп «Копия верна» и слияние PDF-приложений через `pdf-lib` (до 100 документов и 200 МБ).
  - `server/copyCertifyStamp.ts` — векторный штамп 70 × 35 мм, кириллический шрифт, переполнение текста и позиционирование с учётом CropBox/Rotate.
...
- SQL миграция `0019_drop_legacy_telegram_columns.sql` — удаление legacy колонок (admin_users, telegram_user_id); на clean-chain перед `objects.user_id SET NOT NULL` назначает seed/orphan-объектам blocked legacy-owner (`legacy-system-owner@local.invalid`). Подробности инцидента CI Fresh DB: `docs/db-migrations.md`.
...
- `object_responsible_persons`: ответственные лица/подписанты по ролям (ФИО/должность/основание + опционально line/sign); роль `copy_certifier` хранит ФИО и должность заверителя копий. Пустые значения допустимы и отображаются в штампе подчёркиваниями.
...
- `works`: позиции ВОР/ВОИР (код, описание, единицы, плановый объём, синонимы, опционально `lineNo`/`orderIndex`). В графике (`sourceType=works`) основные и вспомогательные строки различаются без отдельного флага в БД (вариант A):
  - **основная** (`isMainWorkPosition`): код с префиксом ГЭСН/ФЕР/ТЕР, либо целый номер (`1`, `10`); при заполненном целом `lineNo` — тоже основная;
  - **вспомогательная**: дробный код/lineNo (`10.1`, `10.2`), префиксы ФССЦ/ФСБЦ/Цена — показываются как раскрываемые подстроки, **не** создаются как `schedule_tasks`;
  - `orderIndex` локален внутри `workCollectionId`; объединённый порядок определяется как `(workCollectionId, orderIndex, code natural, id)`;
  - дробная позиция `N.x` привязывается к ближайшей предшествующей основной `N` только в той же коллекции; для ФССЦ/ФСБЦ/Цена используется предыдущая основная той же коллекции.
  Классификация, collection-aware группировка и сортировка: `shared/workPositionKind.ts`.
...
- `estimate_positions`: позиции сметы (№ п/п, обоснование/шифр, наименование, ед. изм., количество, суммы/примечания). Для RTF ПК РИК сюда также попадают материальные подстроки с дробным `№ п/п` (`3.1`, `17.2`), чтобы график работ показывал их как вспомогательные строки под основной задачей.
- `position_resources`: ресурсы внутри позиции сметы (код/тип, наименование, ед. изм., количество, суммы). Для первой версии RTF ПК РИК ресурсы не импортируются в эту таблицу (`resources: []`).
...
- `schedule_tasks`: задачи графика (полосы Ганта), содержащие `startDate`/`durationDays`/`orderIndex`, номер акта `actNumber`, тип акта `actTemplateId` и поля документации/схем (`projectDrawings`, `normativeRefs`, `executiveSchemes`). 
  - Левая колонка и таймлайн строятся из одного списка видимых задач (`filteredTasks`); раскрытые подстроки входят в расчёт высоты через `client/src/lib/schedule-row-layout.ts`.
  - Горизонтальная фоновая сетка не используется, поскольку высота строки меняется при раскрытии подстрок; вертикальная календарная сетка сохраняется.
...
- **Schedules**: 
  ...
  - `POST /api/schedules/:id/bootstrap-from-works` — создать задачи из ВОР текущего объекта (только основные позиции). Переданные `workIds` пересекаются с коллекциями объекта; пустой массив отклоняется. Полный bootstrap удаляет legacy auxiliary/out-of-scope tasks, при repair восстанавливает source-порядок и возвращает `{ scheduleId, created, skipped, removed }`; частичный bootstrap не выполняет глобальную очистку.
  - `POST /api/schedules/:id/bootstrap-from-estimate` — создать задачи из позиций сметы (только ГЭСН/ФЕР/ТЕР)
