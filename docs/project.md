...
- `works`: позиции ВОР/ВОИР (код, описание, единицы, плановый объём, синонимы, опционально `lineNo`/`orderIndex`). В графике (`sourceType=works`) основные и вспомогательные строки различаются без отдельного флага в БД (вариант A):
  ...
  - **вспомогательная**: дробный код/lineNo (`10.1`, `10.2`), префиксы ФССЦ/ФСБЦ/Цена — показываются как раскрываемые подстроки под последней основной, **не** создаются как `schedule_tasks`.
  Классификация и группировка: `shared/workPositionKind.ts`.
...
- **Schedules**: 
  ...
  - `POST /api/schedules/:id/bootstrap-from-works` — создать задачи из ВОР (только основные позиции; вспомогательные tasks при повторном bootstrap удаляются)
