...
## Задача: Экземпляры материалов → несколько документов качества в акте (вариант D)
- **Статус**: В процессе (план готов, ждут ответы на Q1–Q10)
- **Дата фиксации**: 2026-08-01
- **Описание**: Чтобы в приложения акта можно было класть несколько паспортов одного материала (например, 2 термометра), моделируем каждый паспорт как экземпляр (`material_batches` + binding на партию) и добавляем в задачу несколько строк с разными `batchId`.
- **Шаги выполнения**:
  - [x] Диагностика прода: акт №2, термометр/манометр, один primary, `fileUrl=null` → 422
  - [x] Сравнение вариантов A/B/C/D; выбран D
  - [x] План в `ai_docs/develop/plans/2026-08-01-act-quality-doc-instances.md`
  - [ ] Ответы на уточняющие вопросы Q1–Q10
  - [ ] ADR + реализация INST-001…009
- **Зависимости**: `task_materials`, `material_batches`, `document_bindings`, `generate-acts`, `export-attachments`
