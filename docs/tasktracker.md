...
## Задача: Полировка MCP-агента в Cursor (перед UI B)
- **Статус**: В процессе
- **Дата**: 2026-08-03
- **Описание**: Стабилизировать сквозной сценарий исполнительной документации через Cursor Agent + `/mcp` до разработки отдельного экрана «Ассистент» (вариант B). UI Home («Журнал работ») не менять.
- **Шаги выполнения**:
  - [x] Написан runbook `docs/mcp-cursor-polish-runbook.md`
  - [ ] Gate 0: Cursor MCP auth / `ping` / `list_objects`
  - [ ] Этапы A–H на одном эталонном объекте
  - [ ] Негативы N1–N7
  - [ ] Definition of Done runbook → старт варианта B
- **Зависимости**: TASK-001…012 (`mcp-mvp-plan`), `docs/mcp-pilot-runbook.md`
...
- **Статус**: Завершена
- **Дата фиксации**: 2026-08-02
- **Описание**: Из окна материалов задачи можно создать локальный материал через существующий `MaterialWizard`; после сохранения материал попадает в объект и сразу привязывается к задаче (партия/док качества — при наличии).
- **Шаги выполнения**:
  - [x] Расширить `MaterialWizard` (`onCreated`, `skipSourceStep`, `CreatedMaterialResult`)
...
## Задача: Компактная левая строка графика Ганта
- **Статус**: Завершена (2026-08-01)
- **Описание**: Код позиции вынесен в постоянную левую колонку, объём перенесён в строку метаданных, блок даты и третья строка удалены.
- **Шаги выполнения**:
  - [x] Перестроить основную строку задачи без изменений auxiliary-строк
  - [x] Показывать `—`, если объём не задан
  - [x] Уменьшить высоту строки до 72 px с существующим центрированием полос Ганта
  - [x] Сузить колонку кода до `w-[3ch]` (трёхзначное число)
  - [x] Проверить TypeScript, тесты и production build
- **Детали**: `ai_docs/develop/plans/2026-08-01-gantt-left-row-layout.md`

---

## Задача: ISS-016 — Сортировка карточек актов по возрастанию №
- **Статус**: Не начата (техдолг / UX)
- **Дата фиксации**: 2026-08-01
- **Описание**: На вкладке Акты показывать карточки в порядке возрастания номера акта (сейчас визуально 10 → 9 → 8 → …).
- **Шаги выполнения**:
  - [ ] Проверить callers `getActs` — менять сортировку в storage или только в UI
  - [ ] `actNumber` ASC, NULLS LAST, tie-breaker по `id`
  - [ ] Smoke на объекте с несколькими актами
- **Зависимости**: вкладка Акты / `storage.getActs`
- **Детали**: `ai_docs/develop/issues/ISS-016-acts-list-sort-ascending.md`

---

## Задача: ISS-015 — Массовое назначение параметров задач Gantt (Ctrl+select)
- **Статус**: Не начата (техдолг / UX)
- **Дата фиксации**: 2026-08-01
- **Описание**: С зажатым Ctrl выбирать несколько задач на Gantt (типично с одним номером акта) и массово менять: номер акта, тип акта, дата начала, длительность.
- **Шаги выполнения**:
  - [ ] Multi-select (Ctrl+click) + визуальное выделение
  - [ ] Bulk-edit UI для actNumber / тип акта / start / duration
  - [ ] Persist (batch или multi-patch) + валидация edge cases
  - [ ] Smoke: задачи одного акта → массовая смена параметров
- **Зависимости**: модуль Schedule/Gantt
- **Детали**: `ai_docs/develop/issues/ISS-015-gantt-bulk-edit-ctrl-select.md`
- **Уточнить перед стартом**: только одинаковый actNumber или любой multi-select; Shift-range; аналог Ctrl на tablet
...
## Задача: COPY-CERT — Штамп «Копия верна» на PDF-приложениях
- **Статус**: Завершена локально
- **Дата начала**: 2026-08-01
- **Дата завершения разработки**: 2026-08-01
- **Описание**: Роль заверителя копий в исходных данных и визуальный штамп на каждой странице всех PDF-вложений пакета приложений; титул не штампуется.
- **Шаги выполнения**:
  - [x] Роль `copy_certifier`, API/storage и UI ФИО/должности
  - [x] Штамп 70 × 35 мм, −12°, отступ 10 мм, переполнение и best-effort зернистость
  - [x] Интеграция в `server/actAttachmentsPdf.ts` без изменения исходных файлов
  - [x] Учёт CropBox/Rotate и масштабирование малых страниц
  - [x] Runnable checks и визуальный smoke
  - [x] Документация
- **Проверка**: `npm test` (20/20), `npm run check`, `git diff --check`.
- **Детали**: `docs/tasktrecker-copy-correct.md`
...
- **Зависимости**: DOC-FILE-UPLOAD, act_document_attachments; штамп «Копия верна» завершён (`docs/tasktrecker-copy-correct.md`)
...
- **Описание**: Добавить на экран `/works` импорт локальных сметных расчётов из `.rtf`, сформированных ПК РИК. Файл обрабатывается только в браузере, без загрузки исходного RTF на сервер. Первая версия извлекает метаданные, разделы, основные позиции сметы и материальные подстроки с дробным `№ п/п`, но не импортирует ресурсы в `position_resources`. Результат преобразуется в существующий `ParsedEstimateImportPayload` и после предпросмотра отправляется через действующий `POST /api/estimates/import`; изменения БД и backend-контракта не требуются.
- **Зафиксированные решения**:
  - Парсинг выполняется на клиенте; исходный RTF не передаётся и не сохраняется на сервере.
  - Поддерживаемый источник первой версии — RTF-выгрузка ПК РИК; произвольные RTF-документы не считаются совместимыми.
  - Ресурсы первой версии не разбираются в `position_resources`: в payload передаётся `resources: []`.
  - Материальные строки РИК с дробным `№ п/п` (`3.1`, `6.1`, `17.2`) импортируются как вспомогательные `estimate_positions`, чтобы попадать в график работ как подстроки.
  - Excel-импорт ГРАНД-Сметы сохраняется без изменения поведения.
  - Для РИК создаётся отдельный парсер, а общий API/схема данных повторно используются.
  - Специализированный ограниченный RTF-декодер предпочтительнее полной сторонней библиотеки: он должен обрабатывать только необходимые таблицы РИК и не интерпретировать документ как HTML.
  - **Preview-счётчик**: считать `positionCount` через `parsed.positions.length` (и `sectionCount` через `parsed.sections.length`). Текущий код в `Works.tsx` ошибочно ищет `sections[].positions` — это баг Excel-preview, исправляется в рамках `RIK-RTF-005`.
  - **Warnings/stats**: не расширять `ParsedEstimateImportPayload` и не отправлять на сервер. Хранить только в UI preview-обёртке (`EstimateImportPreview`) и/или в отдельном результате парсера `{ payload, warnings, stats }`, из которого в API уходит только `payload`.
  - **Тесты Web Worker**: автоматизировать чистые функции (`rikRtfTableExtractor`, `rikRtfEstimateParser`) через текущий Node test runner. Worker-обёртку покрывать тонким контрактным тестом на форму сообщений (если возможно без браузера) + обязательный ручной smoke в Chromium; e2e/Playwright в первую версию не добавлять.
  - **Telegram WebView**: только ручная проверка на устройстве/в клиенте Telegram после прохождения `npm run check/test/build` и smoke в Chromium. Playwright/e2e-инфраструктуру не вводить в рамках этой задачи.
- **Эталонные файлы**:
  - `docs/smeta-rtf/02-01-02 Ремонт системы отопления.rtf`
  - `docs/smeta-rtf/02-01-04 Ремонт системы водоснабжения.rtf`
  - `docs/smeta-rtf/02-01-05 Водомерный узел.rtf`
- **Шаги выполнения**:
  - [x] **RIK-RTF-001 — Зафиксировать контракт и ожидаемый результат**
    - Описать формат входа: RTF 1, `ansicpg1251`, табличные строки `\trowd`, границы `\cell`/`\row`.
    - Вручную проверить и зафиксировать для каждого эталона номер/название сметы, разделы и точное количество импортируемых позиций.
    - Уточнить правило идентификатора позиции: использовать колонку `№ п/п`, а служебную колонку `номер` не сохранять как `lineNo`.
    - Зафиксировать, какие строки относятся к позициям, а какие исключаются: ресурсы, коэффициенты, НР/СП, итоги и служебные строки.
  - [x] **RIK-RTF-002 — Реализовать безопасное извлечение таблиц из RTF**
    - Добавить чистый модуль `client/src/lib/rikRtfTableExtractor.ts`, принимающий `ArrayBuffer` и возвращающий нормализованные строки/ячейки.
    - Поддержать CP1251, RTF hex-escape `\'hh`, Unicode-коды `\uN` с `\ucN`, экранированные символы и переносы.
    - Сохранять границы `\cell` и `\row`; игнорировать форматирование, font/color/style tables, `\pict`, `\object` и прочие необязательные destinations.
    - Корректно пропускать `\binN` и вложенные группы, чтобы бинарные данные не попадали в текст сметы.
    - Ввести конфигурируемый лимит размера файла (предварительно 15 МБ), количества строк/ячеек и длины ячейки; при превышении возвращать понятную ошибку.
    - Ввести лимит глубины вложенных RTF-групп (`maxGroupDepth`), чтобы битый RTF не раздувал стек состояний до OOM.
    - Не использовать `innerHTML`, `eval`, выполнение RTF-команд или запись содержимого файла в журналы.
  - [x] **RIK-RTF-003 — Реализовать парсер структуры ПК РИК**
    - Добавить `client/src/lib/rikRtfEstimateParser.ts`, возвращающий `ParsedEstimateImportPayload`.
    - Проверять сигнатуру RTF и строгие признаки экспорта ПК РИК (`Наименование программного продукта` или `ПК РИК`, заголовок ЛСР).
    - Извлекать `estimate.code`, `estimate.name`, `objectName`, `region`, `pricingQuarter` и итоговую сметную стоимость без изменения единиц исходного документа.
    - Находить шапку по колонкам `№ п/п`, `Обоснование`, `Наименование работ и затрат`, `Единица измерения`, `Количество`, не полагаясь только на фиксированные индексы.
    - Распознавать разделы `Раздел N. ...`; позиции до первого явного раздела помещать в синтетический раздел `0 — Без раздела`.
    - Для позиции извлекать `lineNo`, `code`, `name`, `unit`, `quantity`, `orderIndex`, `sectionNumber`; сумму позиции брать из строки `Всего по позиции`, если она однозначно найдена.
    - Импортировать материальные подстроки с дробным `№ п/п` как вспомогательные `estimate_positions`; исключать ресурсы ОТ/ЭМ/М, поправочные коэффициенты, ФОТ, НР, СП, промежуточные итоги и итоги по разделу/смете.
    - Возвращать структуру `{ payload: ParsedEstimateImportPayload, warnings: string[], stats: {...} }`; в `payload.resources` всегда `[]`.
    - Завершать импорт ошибкой, если не найдена шапка или не распознано ни одной позиции; не импортировать частично пустую смету молча.
  - [x] **RIK-RTF-004 — Не блокировать интерфейс при разборе**
    - Вынести запуск RTF-парсера в Web Worker, сохранив чистые функции отдельно для модульного тестирования в Node.
    - Передавать между UI и worker только `ArrayBuffer`, `{ payload, warnings, stats }` и сериализованную безопасную ошибку.
    - Гарантировать завершение worker после успеха, ошибки или размонтирования страницы.
    - Останавливать зависший worker по timeout (60 секунд) и игнорировать late result после unmount/abort.
  - [x] **RIK-RTF-005 — Интегрировать формат в экран смет**
    - Расширить `accept` до `.xlsx,.xls,.rtf`.
    - Добавить диспетчер по расширению: Excel → текущий `parseEstimateWorkbook`, RTF → новый парсер РИК.
    - **Исправить preview-счётчик**: `positionCount = parsed.positions.length`, `sectionCount = parsed.sections.length` (для Excel и RTF одинаково).
    - Расширить `EstimateImportPreview`: `source`, `warnings?`, `stats?`; в `mutateAsync` отправлять только `parsed` (payload без warnings/stats).
    - Валидировать расширение, размер и сигнатуру файла до парсинга; MIME использовать только как дополнительный сигнал из-за ненадёжности браузерного определения RTF.
    - Сохранить текущий двухэтапный поток: разбор → предпросмотр → подтверждение → `POST /api/estimates/import`.
    - В предпросмотре показывать источник `ПК РИК / RTF`, имя файла, число разделов и позиций, а также предупреждение «ресурсы в этой версии не импортируются».
    - Выводить отдельные понятные ошибки для повреждённого RTF, неподдерживаемого экспорта, отсутствующей таблицы и превышения лимита.
    - Сбрасывать значение file input после обработки, чтобы тот же файл можно было выбрать повторно.
  - [x] **RIK-RTF-006 — Добавить автоматические тесты**
    - Модульные тесты RTF-декодера (Node test runner): CP1251, `\'hh`, `\uN`, вложенные группы, `\pict`, `\binN`, границы ячеек/строк и повреждённый ввод.
    - Контрактные тесты парсера на трёх эталонных файлах с проверкой метаданных, разделов, порядка и ключевых позиций.
    - Негативные тесты: пустой файл, переименованный не-RTF, RTF не из РИК, отсутствующая шапка, превышение лимитов и документ без позиций.
    - Регрессионный тест: существующий импорт `.xlsx/.xls` продолжает использовать `parseEstimateWorkbook`; счётчик preview использует плоский `positions`.
    - Проверить, что во всех RTF-результатах `resources` пуст, материальные дробные строки импортируются как вспомогательные позиции, а служебные строки не превращаются в позиции.
    - Worker: без Playwright; максимум контракт сообщений + ручной smoke в Chromium.
  - [ ] **RIK-RTF-007 — Проверить UX, производительность и совместимость**
    - [x] Автоматизируемый smoke: `npm run check`, `npm test`, `npm run build`.
    - [ ] Ручной preview/confirm/import трёх эталонов в Chromium.
    - [ ] Telegram WebView — только ручная проверка (без Playwright/e2e в этой задаче).
    - Проверить, что обработка файла максимального разрешённого размера не блокирует UI и не оставляет активный worker.
    - Сопоставить импортированные позиции с исходными RTF вручную по началу, середине, концу и границам разделов.
  - [x] **RIK-RTF-008 — Актуализировать документацию после реализации**
    - Обновить `docs/project.md`: клиентский поток RTF РИК, ограничения первой версии и взаимодействие с существующим API.
    - Обновить `docs/changelog.md`: новый пользовательский формат импорта.
    - При необходимости добавить краткую инструкцию по корректной выгрузке RTF из ПК РИК.
    - Перевести эту задачу в статус `Завершена` только после ручного Chromium/Telegram smoke.
- **Критерии приёмки**:
  - Все три эталонных RTF распознаются на клиенте и доходят до существующего окна подтверждения импорта.
  - Номер, название, разделы, основные позиции, материальные подстроки, единицы и количества совпадают с исходными сметами в пределах зафиксированного контракта.
  - Материальные строки с дробным `№ п/п` создают вспомогательные `estimate_positions`; ресурсы ОТ/ЭМ/М, коэффициенты и строки итогов не создают ложных позиций; `resources` всегда равен `[]`.
  - Исходный RTF не отправляется на сервер; сервер получает только существующий JSON payload.
  - Ошибочные и неподдерживаемые файлы отклоняются до API-вызова с понятным сообщением.
  - Импорт Excel-смет работает без регрессий.
- **Риски и меры**:
  - В разных версиях РИК возможны отличия колонок — использовать поиск по заголовкам и хранить предупреждения, а не только жёсткие индексы.
  - RTF может содержать большой объём форматирования/изображений — пропускать destinations и бинарные блоки, применять лимиты, выполнять разбор в worker.
  - MIME для `.rtf` нестабилен — проверять расширение вместе с сигнатурой `{\rtf`.
  - Числовой формат содержит запятые и неразрывные пробелы — нормализовать только пробелы, не теряя исходный десятичный разделитель, совместимый с текущим API.
- **Не входит в первую версию**: импорт в `position_resources`; `.doc`/`.docx`; универсальная поддержка RTF других сметных программ; OCR; изменение схемы БД; серверное хранение исходного файла.
- **Зависимости**: `client/src/pages/Works.tsx`, `client/src/lib/estimateParser.ts`, `client/src/hooks/use-estimates.ts`, `shared/routes.ts`, Web Worker/Vite, эталоны `docs/smeta-rtf/*.rtf`.
...
- **Дата начала**: 2026-07-24
- **Дата завершения**: 2026-07-24
- **Описание**: Перед архивацией TelegramJurnalRabot перенести в канонический `idgenerator` документы, которых ещё не было в репозитории: Odoo styleguide и трекер git-ремонта.
- **Шаги выполнения**:
  - [x] Скопировать `docs/stilegidtopus/styleguide-odoo-tjr.md` и `.html`
  - [x] Скопировать `docs/tasktrecker_git_remont.md`
  - [x] Обновить `docs/changelog.md`
- **Зависимости**: канон-репозиторий `idgenerator`
