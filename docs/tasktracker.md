...
- **Статус**: В процессе (почти все решения; остался Q7)
- **Дата фиксации**: 2026-08-01
- **Описание**: D′ multi-row; unique `(task, material, document)`; дубль паспорта запрещён; п.3 — перечень в одной ячейке; выбор в графике и карточке акта; клик по строке без PDF → upload на месте.
- **Шаги выполнения**:
  ...
  - [ ] Ответ на Q7 (автовыбор при добавлении) или «бери рекомендацию: multi-select»
