...
- **Описание**: Добавить на экран `/works` импорт локальных сметных расчётов из `.rtf`, сформированных ПК РИК. Файл обрабатывается только в браузере, без загрузки исходного RTF на сервер. Первая версия извлекает метаданные, разделы и позиции сметы, но не импортирует ресурсы ОТ/ЭМ/М. Результат преобразуется в существующий `ParsedEstimateImportPayload` и после предпросмотра отправляется через действующий `POST /api/estimates/import`; изменения БД и backend-контракта не требуются.
- **Зафиксированные решения**:
  ...
  - Ресурсы первой версии не разбираются: в payload передаётся `resources: []`.
...
- **Шаги выполнения**:
  ...
  - [x] **RIK-RTF-003 — Реализовать парсер структуры ПК РИК**
    ...
    - Исключать строки ресурсов ОТ/ЭМ/М, поправочных коэффициентов, ФОТ, НР, СП, промежуточных итогов и итогов по разделу/смете.
  ...
  - [x] **RIK-RTF-006 — Добавить автоматические тесты**
    ...
    - Проверить, что во всех RTF-результатах `resources` пуст и служебные строки не превращаются в позиции.
...
- **Критерии приёмки**:
  ...
  - Номер, название, разделы, позиции, единицы и количества совпадают с исходными сметами в пределах зафиксированного контракта.
  - Ресурсы, коэффициенты и строки итогов не создают ложных позиций; `resources` всегда равен `[]`.
...
- **Не входит в первую версию**: импорт ресурсов; `.doc`/`.docx`; универсальная поддержка RTF других сметных программ; OCR; изменение схемы БД; серверное хранение исходного файла.
