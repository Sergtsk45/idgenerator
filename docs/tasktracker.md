...
- **Статус**: В процессе (план уточнён, ждут ответы на Q1–Q7)
- **Дата фиксации**: 2026-08-01
- **Описание**: В приложения акта нужно класть несколько документов качества одного материала (2 паспорта или паспорт+сертификат). Номер партии не связывает и не ограничивает выбор docs — оси ортогональны. Рекомендация плана: D′ (несколько строк `task_materials` с разными `qualityDocumentId`, `batchId` optional).
- **Шаги выполнения**:
  - [x] Диагностика прода: акт №2, термометр/манометр, один primary, `fileUrl=null` → 422
  - [x] Сравнение вариантов; первая редакция D завязала экземпляр на партию
  - [x] Уточнение заказчика: партия ⟂ документы качества
  - [x] План переписан: `ai_docs/develop/plans/2026-08-01-act-quality-doc-instances.md`
  - [ ] Ответы на Q1–Q7
  - [ ] ADR + реализация INST-001…009
- **Зависимости**: `task_materials`, `document_bindings`, `generate-acts`, `export-attachments` (партии — опциональный атрибут)
