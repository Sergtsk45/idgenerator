...
- **Шаги выполнения**:
  - [x] 18.1 Header: `← Акт №N` через ResponsiveShell `onBack`
  - [x] 18.2 Status badge + progress bar (3 шага: черновик / в работе / принято)
  - [x] 18.3 Accordion-секции: основные данные, работы, материалы, документы, нормативы
  - [x] 18.4 Sheet снизу для выбора шаблонов PDF (КС-2, АОСР упрощённый, АОСР полный)
  - [x] 18.5 Кнопка «Экспорт PDF» → анимированный прогресс-бар генерации
  - [ ] 18.6 Inline-редактирование полей (OdooForm inputs) — отложено
  - [x] 18.7 Warning-блоки для отсутствующих дат и чертежей
...
- **Шаги выполнения**:
  - [x] 19.1 OdooCard + OdooBadge (Доки ✓ / Нет доков) + поиск
  - [x] 19.2 Infinite scroll (IntersectionObserver, PAGE_SIZE=20)
  - [x] 19.3 Правая панель lg+: OdooCard с метриками (партии, документы, кач. доки) + Badge
  - [ ] 19.4 Wizard добавления материала (4 шага) — уже есть MaterialWizard, stepper не добавлен
  - [ ] 19.5 Кнопка «📷 Скан» — уже есть в 16.5
  - [x] 19.6 OdooEmptyState для пустого списка и нулевых результатов поиска
...
- **Шаги выполнения**:
  - [x] 20.1 OdooCard + Badge (тип) + название + дата
  - [x] 20.2 PillTabs фильтр по типу документа + Select для scope
  - [x] 20.3 Infinite scroll (IntersectionObserver, PAGE_SIZE=20)
  - [x] 20.4 Sheet (вместо Dialog) с OdooForm-формой добавления
  - [x] 20.5 OdooEmptyState для пустого реестра
