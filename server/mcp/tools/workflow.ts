...
export function registerWorkflowTools(server: McpServer, authResolution: McpAuthResolution): void {
  ...
  server.registerTool(
    ...
    withToolLogging("create_execution_workflow", userIdForLog, async (args) => {
      try {
        const auth = requireAuth(authResolution);
        return toolSuccess(await createExecutionWorkflow(auth, args));
      } catch (err) {
  ...
  server.registerTool(
    ...
    withToolLogging("get_execution_workflow", userIdForLog, async (args) => {
      try {
        const auth = requireAuth(authResolution);
        return toolSuccess(await getExecutionWorkflow(auth, args.workflowId));
      } catch (err) {
        return toolError(err);
      }
    }),
  ...
  server.registerTool(
    ...
    withToolLogging("get_missing_workflow_inputs", userIdForLog, async (args) => {
      try {
        const auth = requireAuth(authResolution);
        return toolSuccess(await getMissingWorkflowInputs(auth, args.workflowId));
      } catch (err) {
        return toolError(err);
      }
    }),
  ...
  server.registerTool(
    ...
    withToolLogging("set_workflow_input", userIdForLog, async (args) => {
      try {
        const auth = requireAuth(authResolution);
        return toolSuccess(
