...
test("documents.list accepts project/global/all view modes", () => {
  ...
  assert.equal(api.documents.list.input?.parse({ scope: "global" } as any).viewMode, "project");
});
...
test("documents.setScope exposes expected method/path and only accepts publishing to global", () => {
  ...
  assert.throws(() => api.documents.setScope.input.parse({ scope: "project" }));
